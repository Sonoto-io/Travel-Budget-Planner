<template>
    <div class="background" >
      <img :src="planeImg" alt="plane" class="plane" 
      :style="'top: ' + (20 + Math.round(Math.random() * 60))+'%'"/>
      <div v-for="n in nbClouds" :key="'cloud-'+n">
        <img :src="clouds[Math.floor(Math.random() * clouds.length)]" alt="cloud" class="cloud"
            :style="'top: ' +  (10 + Math.round(Math.random() * 100))+'%;'+
      ...
      <div v-for="n in nbMounts" :key="'mount-'+n">
        <img :src="mounts[Math.floor(Math.random() * mounts.length)]" alt="mount" class="mount"
            :style="'bottom: ' + (-105 + Math.round(mountValues[n - 1] * 100))+'%;'+
...
@keyframes day-cycle {
  ...
  25% { background: rgba(255, 128, 0, 0.3); }
  50% { background: rgba(0, 0, 50, 0.5); }
  75% { background: rgba(0, 247, 255, 0.3); }
