...
export const useCountryStore = defineStore('countries', () => {
    ...
    async function setCurrentCountryData(countryShortname: string) {
        if (countryList.value.length > 0) {
            ...
            currentMainCurrency.value = (await getCurrency(currentCountry.value.main_currency)).data
            ready.value = true
        }
    }
    ...
    return { countryList, currentCountry, currentMainCurrency, ready, setCurrentCountryData, isCurrentCountrySet, }
})